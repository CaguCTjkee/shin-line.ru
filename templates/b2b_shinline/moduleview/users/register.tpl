...
<form method="POST" action="{$router->getUrl('users-front-register')}">
    ...
    {hook name="users-registers:form" title="{t}Регистрация:форма{/t}"}
            ...
            <table class="formTable tabFrame{if $user.is_company} thiscompany{/if}" id="profile">
                ...
                <tr>
                    ...
                    <td class="value">
                        ...
                        <div class="help">Например: +7 918 00011222. Можно указывать несколько номеров, разделяя их
                            запятой
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="value">
                        <div class="ib">
                            <input type="password" name="openpass"
                                   {if count($user->getErrorsByForm('openpass'))}class="has-error"{/if}>
        ...
        <div class="footer-dialog clearfix">
            <button type="submit" class="formSave submit-button">Сохранить</button>
        </div>
    {/hook}
</form>