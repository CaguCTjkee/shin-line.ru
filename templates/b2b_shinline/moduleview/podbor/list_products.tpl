    ...
    {if count($list)}
        ...
        <table class="table table-striped table-products">
            ...
            <tr class="info">
                <td>Фото</td>
                <td>Код</td>
                <td class="sortable">Название</td>
                <td class="sortable">Остаток</td>
                <td>Срок доставки</td>
                <td class="sortable">
            ...
            {foreach $list as $product}
                ...
                {$product_code=$product->getPropertyValueByTitle('Код')}
                ...
                {foreach $stick_info.warehouses as $warehous}
                    ...
                    {/if}
                {/foreach}
                <tr class="clearfix {if $thorns|trim == 'да'}thorns{/if} {if $product@index > 30}hidden{/if}"
                    data-id="{$product.id}">
                    <td class="main-image">
                        <a href="{$product->getUrl()}" class="inDialog">
                            <img {if $product@index > 30}data-{/if}src="{$main_image->getUrl(250, 270, 'xy')}"
                                 class="middlePreview" alt="{$main_image.title|default:"{$product.title}"}"/>
                        </a>
                    </td>
                    <td class="product-barcode">
                        <span class="visible-xs-inline">Код: </span>
                        {if $product_code}{$product_code}{else}-{/if}
                    </td>
                    <td class="product-title clearfix" data-sortable="{$product.title}">
                        <a href="{$product->getUrl()}" class="inDialog">
                            {$product.title}
                        </a>
                        {if $current_user->inGroup('admins')}
                            <a href="//shin-line.ru/admin/catalog-ctrl/?id={$product.id}&do=edit" target="_blank">
                                <i class="glyphicon glyphicon-pencil"></i>
                            </a>
                        {/if}
                    </td>
                    <td class="product-num" data-sortable="{$product.num}">
                        ...
                        {else}
                            <span class="text-danger hidden-xs">мы сообщим о поступлении</span>
                            <span class="text-danger visible-xs-inline">
                                <i class="sicon sicon-info" data-toggle="tooltip" data-placement="top"
                                   title="Мы сообщим о поступлении"></i>
                            </span>
                        {/if}
                    </td>
                    <td class="product-delivery">
                        ...
                        {else}
                            <span class="text-danger visible-xs-inline">Мы сообщим о поступлении</span>
                            <span class="text-danger hidden-xs">-</span>
                        {/if}
                    </td>
                    <td class="product-season" data-sortable="{$product_type}">
                    ...
                    <td class="product-cost" data-sortable="{$product->getCost(null, null, false)}">
                        ...
                        <div class="amount-pick {if $product.num == 0}disabled{/if}">
                            ...
                            <div class="amount-quick-pick">
                                ...
                                <input type="text" class="text-amount"
                                       value="{if $product.num > 4}5{else}{$product.num}{/if} шт">
                                <button type="button" class="btn btn-primary inc-pick">+</button>
                            </div>
                        </div>
                    </td>
                    <td class="product-cart">
                        <a data-href="{if $product.num > 0}{$router->getUrl('shop-front-cartpage', ["add" => $product.id])}{else}{$router->getUrl('shop-front-reservation', ["product_id" => $product.id])}{/if}"
                           class="{if $product.num > 0}addToCart noShowCart{else}inDialog{/if}">
                            {if $product.num > 0}
                                <i class="sicon sicon-cart"></i>
                            {else}
                                <i class="glyphicon glyphicon-earphone" data-toggle="tooltip" data-placement="top"
                                   title="Заказать"></i>
                            {/if}
                        </a>
                    </td>
